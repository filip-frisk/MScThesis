...
def plot_one_physical_variable(df, physical_variable, unit, SIGNAL, BACKGROUND, cut,DATA_FILENAME_WITHOUT_FILETYPE, OVERFLOW_UNDERFLOW_PERCENTILE,BINS,PLOT_RELATIVE_FOLDER_PATH,PLOT_TYPE,SIGNAL_ENVELOPE_SCALE):
    ...
    if PLOT_TYPE == 'postfit': # TODO: rebuild this function to be more general and not only for postfit
        ...
        fig_combined, (ax_top, ax_bottom) = plt.subplots(2, 1, figsize=(8,6), height_ratios = [3, 1])
        ...
        ax_top.set_ylabel(f'Events/{bin_width:.2f} {unit}')
        ax_top.legend(loc='upper right')
        ax_top.set_xticks([]) # Remove x-ticks
    

        atlasify(atlas = True, axes=ax_top,subtext = subtext_latex + " with Testdata: " + str(int(sum(total_hist)))+ f" events with ca. {int(events_per_label[SIGNAL[0]])} signal events from {int(MCsamples_per_labels[SIGNAL[0]])} MC Samples." , sub_font_size = 8)
